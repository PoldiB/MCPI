...
def replacenear(oldIDList, newIDList, oldDataList, newDataList, radius, x, y, zf):
    x1 = x - radius
    x2 = x + radius
    y1 = y - radius
    y2 = y + radius
...
def toggle_world():
    ...
    if worldType > 3:
        worldType = 1
    if worldType == 1:
        print("Toggled to void world")
        mc.setBlocks(-128, -64,-128, 128, 30, 128, 0)
        mc.player.setPos(0,10,0)
        mc.setBlocks(-10, 0, -10, 10, 0, 10, 1)
        print("Platform Created At (0, 0). Teleporting you there now.")
    if worldType == 2:
        print("Toggled to small superflat")
        mc.setBlocks(-300, -64, -300, 300, -64, 300, 7)
        mc.setBlocks(-300, -63, -300, 300, -62, 300, 3)
        mc.setBlocks(-300, -61, -300, 300, -61, 300, 2)
        x, y, z = mc.player.getPos()
        mc.player.setPos = x, -60, z
    if worldType == 3:
        print("Toggled to large superflat")
